...
export function readRevealContext(
  target: Element,
  ...
  const viewports = chain.map((surface, index) => {
    ...
    return effectiveViewport(
      surfaceMetrics.viewport,
      surfaceMetrics.padding,
      isWindow(surface) ? safeArea : resolveInsets(),
      padding,
    );
  });
  ...
  return {
    ...
    viewports,
    combinedViewport,
    visibility: measureVisibility(targetRect, combinedViewport),
  };
}
