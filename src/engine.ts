...
export function createScrollEngine(options: ScrollEngineOptions = {}): ScrollEngine {
  ...
  const executeTo = async (
    ...
    try {
      ...
      return await executor.execute(
        step,
        resolveMotion(scrollOptions.motion),
        scrollOptions.settle ?? {},
        operation.signal,
      );
    } finally {
      operation.cleanup();
  ...
  return {
        ...
        if (revealOptions.settle !== false) {
          for (const step of executed) {
            await executor.execute(
              { ...step, from: step.actualTo, to: step.actualTo, axes: [] },
              { type: "instant" },
              revealOptions.settle ?? {},
              operation.signal,
            );
          }
        }
    ...
    by: (surface, delta, scrollOptions = {}) => {
      ...
      return executeTo(
        surface,
        { x: current.x + (delta.x ?? 0), y: current.y + (delta.y ?? 0) },
        scrollOptions,
      );
    },
