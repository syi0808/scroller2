...
export class Animator {
  ...
  animate(
    ...
    return new Promise<Point>((resolve, reject) => {
      const active: ActiveMotion = {
        ...
        animated: from,
        previous: from,
        duration: this.duration(from, target, motion),
        motion,
        resolve,
        reject,
        callback: () => false,
  ...
  private duration(from: Point, to: Point, motion: MotionOptions): number {
    return motion.type === "tween"
      ? resolveDuration(motion.duration, Math.hypot(to.x - from.x, to.y - from.y))
