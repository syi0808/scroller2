import type { Alignment, Point, Rect, ScrollMetrics, ScrollPlanStep, ScrollSurface, VisibilityPolicy } from "../core/types.js"
...
export function planSurfaceReveal(input: SurfacePlanInput): ScrollPlanStep {
  ...
  const deltaX = satisfied || !input.metrics.axes.x ? 0 : alignmentDelta(input.target.left, input.target.right, input.viewport.left, input.viewport.right, input.inline)
  const deltaY = satisfied || !input.metrics.axes.y ? 0 : alignmentDelta(input.target.top, input.target.bottom, input.viewport.top, input.viewport.bottom, input.block)
