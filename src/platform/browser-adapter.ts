...
export class BrowserScrollPlatform implements ScrollPlatform {
  ...
  getScrollMetrics(surface: ScrollSurface): ScrollMetrics {
    ...
    return {
      ...
      max: {
        x: rightToLeft ? 0 : horizontalRange,
        y: Math.max(0, surface.scrollHeight - surface.clientHeight),
      },
      axes: {
        x: surface.scrollWidth > surface.clientWidth,
        y: surface.scrollHeight > surface.clientHeight,
      },
      padding: style
        ? styleInsets(style, "scrollPadding")
  ...
  readScroll(surface: ScrollSurface): Point {
    return isWindow(surface)
      ...
      : { x: surface.scrollLeft, y: surface.scrollTop };
  }
