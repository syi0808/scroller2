...
export function measureVisibility(target: Rect, viewport: Rect): VisibilitySnapshot {
  ...
  return Object.freeze({
    ...
    targetArea,
    visibilityRatio,
    fullyVisible: target.top >= viewport.top && target.right <= viewport.right && target.bottom <= viewport.bottom && target.left >= viewport.left,
  })
}

export function satisfiesVisibility(snapshot: VisibilitySnapshot, policy: VisibilityPolicy): boolean {
